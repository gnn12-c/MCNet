...
class SS2DConv_K1(nn.Module):
    def __init__(
        ...
        self.in_proj = nn.Linear(d_model, d_conv_input * 2, bias=bias, dtype=torch.float32)
        ...
        self.x_proj = (
            nn.Linear(d_sequence_model, (self.dt_rank + self.d_state * 2), bias=False, dtype=torch.float32),
        )
        # (K=4, N, inner)
        ...
        self.dt_projs = (
            self.dt_init(self.dt_rank, d_sequence_model, dt_scale, dt_init, dt_min, dt_max, dt_init_floor),
        )
        # (K=4, N, inner)
        ...
        self.out_proj = nn.Linear(d_conv_input, d_model, bias=bias, dtype=torch.float32)
        self.dropout = nn.Dropout(dropout) if dropout > 0. else None

    @staticmethod
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4):
        dt_proj = nn.Linear(dt_rank, d_inner, bias=True, dtype=torch.float32)
        ...
        dt = torch.exp(
            torch.rand(d_inner, dtype=torch.float32) * (math.log(dt_max) - math.log(dt_min))
            + math.log(dt_min)
...
class Unfold(nn.Module):
    def __init__(self, kernel_size=3, mode="full"):
        ...
        weight = weight.reshape(kernel_size**2, 1, kernel_size, kernel_size) # 9个 k x k 的卷积核
...
class SS3DConv(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.act = nn.SiLU()
        self.unfold = Unfold3D(d_conv, mode=conv_mode)
        self.fold = nn.Linear(d_sequence_model, d_conv_input, bias=bias)
...
class Unfold3D(nn.Module):
    ...
    def forward(self, x):
        if self.mode == "full":
            ...
            out = UnfoldOp3D.apply(x, self.weight)
        return out


class UnfoldOp3D(autograd.Function):
    mode = "scaled"
